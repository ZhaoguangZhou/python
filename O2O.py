# -*- coding: utf-8 -*-
"""
Created on Thu Feb 29 14:47:57 2024
...
def get_online_feature(df):
    ...
    #各用户 线上领卷数
    f=get_data_ss(data)
    f['received_online_num_every_user']=1
    f=f[['User_id','received_online_num_every_user']]
    pivot=pd.DataFrame(
        pd.pivot_table(f,index=['User_id'],
                       values=['received_online_num_every_user'],
                       aggfunc=len)).reset_index()
    user_feature=pd.merge(user_feature, pivot,on='User_id',how='left')
...
def model_xgb(train, test):
    ...
    # 训练
    model = xgb.train(params, dtrain, num_boost_round=128, evals=watchlist)
